...
def plot_trend_fit(
    ...
    *,
    ...
    """Plot actual time series with fitted trend."""
    if ax is None:
        _, ax = plt.subplots(figsize=figsize)

    sns.lineplot(x=y_true.index, y=y_true, label="Actual", ax=ax, alpha=0.9)
    sns.lineplot(x=y_pred.index, y=y_pred, label="Trend", ax=ax, linestyle="--")
